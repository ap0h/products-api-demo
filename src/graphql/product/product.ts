...
import { getProducer } from '@repositories/producer-repository';
  ...
  const ProductType = new GraphQLObjectType({
    ...
    fields: () => ({
      ...
      vintage: { type: new GraphQLNonNull(GraphQLString), description: 'Product vintage' },
      name: { type: new GraphQLNonNull(GraphQLString), description: 'Product name' },
      producerId: { type: GraphQLID, description: 'ID of the associated producer' },
      producer: {
        type: ProducerType,
        resolve: async (parent: Product) => {     
  ...
  const ProductByProducer = new GraphQLObjectType({
    ...
    fields: () => ({
      ...
      vintage: { type: new GraphQLNonNull(GraphQLString), description: 'Product vintage' },
      name: { type: new GraphQLNonNull(GraphQLString), description: 'Product name' },
      producerId: { type: GraphQLString, description: 'ID of the associated producer' },
    }),
  });
...
const CreateProductInputType = new GraphQLInputObjectType({
    ...
    fields: () => ({
      vintage: { type: new GraphQLNonNull(GraphQLString), description: 'Product vintage' },
      name: { type: new GraphQLNonNull(GraphQLString), description: 'Product name' },
      producer: { type: new GraphQLNonNull(CreateProducerInputType), description: 'Producer' },
    }),
});
...
const DeleteProductsInputType = new GraphQLInputObjectType({
    ...
    fields: () => ({
      productIds: { type: new GraphQLList(GraphQLID), description: 'Product IDs to delete' },
    }),
});
...
export { ProductType, ProductByProducer, CreateProductInputType, ProductOutputType, UpdateProductInputType, DeleteProductsInputType, DeleteProductsOutputType };
